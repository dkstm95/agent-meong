...
목표로 합니다. 오브젝트는 익명이며 프롬프트, 응답, 작업 이름과 채팅 제목을
표시하지 않습니다.
...
- **움직임**: 에이전트가 활동 중입니다.
- **고리**: 사용자의 확인이 필요합니다.
- **바깥으로 번지는 파동**: 한 번의 최상위 agent turn이 끝났습니다. 전체 작업의
  성공을 뜻하지는 않습니다.
...
- 각 오브젝트의 몸 색과 형태는 그 에이전트 자신의 현재 상태를 나타내며, 상태가
  바뀌면 해당 오브젝트의 색도 바뀝니다.
- 메뉴 막대 아이콘은 전체 상태를 한 우선순위 색으로 합치지 않습니다. 하나의 가상
  Agent Key처럼 최근 에이전트 상태 전환을 짧게 순서대로 보여줍니다. 통통 움직임은
  별도 신호로, 하나 이상의 에이전트가 활동 중임을 뜻합니다.
- 부모·자식 관계는 고정 색이 아니라 탄생·흡수와 가까운 움직임으로 표현합니다.
  색을 구분하기 어려워도 상태 형태와 VoiceOver로 같은 의미를 알 수 있습니다.
- 한 번의 최상위 agent turn이 끝나면 메뉴 막대 신호와, 앱을 실행 중인 동안
  확인할 때까지 해당 가족에 남는 종료 흔적이 나타납니다.
- 활동과 오브젝트가 줄어드는 모습으로 일이 마무리되는 흐름을 느낄 수 있습니다.

메뉴 막대의 모든 색과 형태는 [색상 읽기](#색상-읽기)에서 한눈에 확인할 수 있습니다.

이 앱은 상세 로그나 생산성 대시보드가 아닙니다. 프롬프트, 응답, 명령, 파일 경로와
tool input/output을 저장·로그하거나 macOS 앱으로 전송하지 않습니다. 작업의
내용 대신 최소한의 활동 신호만 시각화합니다.
기능과 디자인의 판단 기준은 [Product Principles](docs/product-principles.md)에 있습니다.

## 빠른 시작

현재는 GitHub 소스 설치만 제공합니다. macOS 14 이상, Xcode Command Line
Tools와 Swift 6, 그리고 `/hooks`를 지원하는 로컬 Codex가 필요합니다. `sudo`는
필요하지 않고, 설치 후에는 로그인 시 자동으로 실행됩니다.

1. `⌘ Space`로 Spotlight를 열어 `터미널`을 검색하고 Terminal을 엽니다.
2. 처음 설치하는 Mac이라면 다음 명령을 먼저 실행합니다.

   ```bash
   xcode-select -p
   ```

   `/Library/Developer/CommandLineTools`처럼 경로가 나오면 3번으로 가세요. 오류가 나오면
   `xcode-select --install`을 실행하고 표시되는 설치를 **끝까지 마친 뒤** 3번을
   진행하세요.
3. 아래 코드 블록을 **세 줄 모두** 복사해 Terminal에 붙여넣고 `Return`을 누릅니다.
   ...
   첫 release build 중에는 몇 분간 많은 글자가 출력될 수 있습니다. `Building agent-meong`으로
   시작하는 줄이 보이면 정상이니 Terminal을 닫지 말고 기다리세요. 마지막에
   `Installed agent-meong at`으로 시작하는 줄이 보이면 설치가 완료된 것입니다.

   `destination path ... already exists`가 보이면 이미 소스 폴더가 있는 것입니다.
   폴더를 바로 지우지 말고 [업데이트](#업데이트) 명령을 실행하세요.
4. 자동으로 열린 agent-meong에서 `연결 시작`을 누릅니다.
5. agent-meong이 새 Terminal을 열고 Codex를 자동으로 실행합니다. `Hooks need review`가
   나타나면 `Review hooks`를 선택합니다. 정상 흐름에서는 `/hooks`를 직접 붙여넣고
   `Return`으로 실행하는 단계가 없습니다.
6. `User config`에서 앱이 안내하는 7개 lifecycle event와 각 command 정의를 확인하고
   신뢰합니다. Codex에는 별도의 `agent-meong` hook 이름이 보이지 않는 것이 정상입니다.
   앱이 `다른 검토 대기 hook은 없습니다`라고 확인한 경우에는 `Trust all`을 사용할 수
   있습니다. 앱이 다른 대기 항목 수를 표시하면 7개만 개별 신뢰하세요.
7. 메뉴 막대 아이콘을 열어 `Codex · 활동 대기`가 보이면 **설정이 완료**된 것입니다.
   검토용 Terminal은 닫아도 됩니다. 평소처럼 로컬 Codex를 사용하면 첫 활동부터
   자동으로 보입니다. 바로 시험해 보고 싶을 때만 열린 CLI에 짧은 요청을 보내세요.

`Codex · 활동 대기`는 hook 설정과 신뢰가 끝났지만 아직 실제 활동은 받지 않았다는
뜻입니다. 첫 활동 신호 뒤에는 `Codex · 방금`과 움직이는 오브젝트가 보입니다.
연결 전에 열어 두었던 Codex App·CLI는 한 번 완전히 종료하고 다시 열어야 합니다.
원격·웹 작업은 표시되지 않습니다. [연결만 끊기](#codex-연결-해제)와
...
- `/hooks`를 지원하는 최신 Codex App, ChatGPT 데스크톱 앱 안의 Codex 또는 Codex CLI
- 관찰할 로컬 Codex 작업
...
기다리세요. Swift가 6.x가 아니면 `시스템 설정 > 일반 > 소프트웨어 업데이트`에서
macOS와 Xcode Command Line Tools의 사용 가능한 업데이트를 설치합니다.

ChatGPT·Codex 앱이 없고 `codex --version`도 실패하거나 `/hooks`가 없다면 OpenAI의
...
위 블록을 전체 복사해 Terminal에 붙여넣으면 됩니다. 스크립트는 build 필수 환경을
확인하고 release build와 ad-hoc 서명을 검증한 뒤
...
업데이트가 없으므로 `$HOME/agent-meong` 소스 폴더는 업데이트와 완전 제거를 위해
보관하세요.

`destination path ... already exists`는 이 폴더가 이미 있다는 뜻입니다. 이전에
agent-meong을 받았다면 [업데이트](#업데이트)를 실행하세요. 다른 파일이나 끝나지
않은 다운로드라면 Finder에서 해당 폴더의 이름을 바꾼 뒤 다시 설치하세요. 내용을 확인하지
않고 폴더를 지우지 마세요.
...
2. agent-meong이 기존 설정을 보존하면서 기본 `~/.codex`에 lifecycle hook과 adapter를
   설치하고, 검토를 위한 새 Terminal과 Codex CLI를 자동으로 엽니다. `/hooks`는 예비
   복구 수단으로 clipboard에도 복사합니다.
3. `Hooks need review`가 보이면 `Review hooks`를 선택합니다. 정상 흐름에서는 `/hooks`를
   붙여넣고 `Return`으로 실행하는 단계가 없습니다. 만약 검토 화면 대신 일반 입력 화면만
   나타나면, 앱이 이미 복사한 `/hooks`를 `⌘V` → `Return`으로 여는 예비 경로를
   사용하세요.
4. `User config`에서 `UserPromptSubmit`, `PreToolUse`, `PermissionRequest`,
   `PostToolUse`, `Stop`, `SubagentStart`, `SubagentStop`의 command 정의 7개가 같은
   agent-meong forwarder를 가리키고 활성화되어 있는지 확인한 뒤 신뢰합니다. 다른
   handler는 변경하지 않습니다. 앱이 다른 검토 대기 hook이 `0개`라고 확인한 경우에는
   `Trust all`을 사용할 수 있습니다. 다른 수가 보이면 7개만 개별 신뢰하세요.
...
승인 뒤 `Codex · 활동 대기`가 보이면 연결 설정은 완료된 것입니다. 검토용 Terminal은
닫아도 되며, 열린 CLI에 별도의 확인 요청을 보내는 것은 선택 사항입니다. 바로
시험하고 싶다면 그 CLI에 `“연결 확인”이라고만 답해줘`처럼 파일을 바꾸지 않는 짧은
요청을 보내세요.

연결 전에 이미 열려 있던 Codex App·CLI를 사용하려면 완전히 종료하고 다시 연 뒤
**새 로컬 작업**을 시작하세요. 일반 ChatGPT 대화와 Codex 원격·웹 작업은 표시되지
않습니다.
...
- 각 오브젝트의 몸 색은 자신의 현재 상태에 따라 독립적으로 바뀝니다. 동시에 실행되는
  모든 에이전트의 현재 상태는 Meong Space에서 함께 볼 수 있습니다. 가족 관계는 크기,
  가까운 움직임, 탄생과 흡수로 표현합니다.
- 메뉴 막대의 몸 색과 형태는 가장 최근에 상태가 바뀐 에이전트를 보여줍니다. 빠르게
  이어진 서로 다른 에이전트의 변화는 짧게 순서대로 표시합니다. 확인 필요와
  실패가 남아 있으면 최근 변화의 몸 색과 별개로 고리 또는 마름모가 유지됩니다.
- 드물게 보이는 분절 고리는 `상태 불확실`, 열린 호는 `종료됨`, 이중 후광은 `완료`,
  가로 막대는 `취소됨`, 마름모는 `실패 보고`를 뜻합니다. 완료·취소·실패는 Codex가
  결과를 명시한 경우에만 표시합니다.
- 우상단 `?` 아이콘을 누르면 세 핵심 신호, 도구 활동 점, 위의 모든 상태 형태를
  아이콘으로 다시 확인할 수 있습니다. 안내 안의 `×` 또는 `Escape`로 장면만 다시
  볼 수 있고, 바깥을 클릭하면 Meong Space 전체가 닫힙니다.
- 도구 시작이 관찰되면 채운 점이 바깥으로, 종료가 관찰되면 빈 점이 안쪽으로 짧게
  움직입니다. 2~3개가 겹쳐도 짧은 시간에 이어진 이벤트일 뿐 실행 중인 도구 수가
  아닙니다. 점이 나타나는 각도는 장식이며 도구 종류를 뜻하지 않습니다. 도구가 계속
  실행 중이라고 추정하거나 상세 payload를 보여주지는 않습니다.
...
- 아이콘을 오른쪽 클릭하면 상태, `멍 보기`, `도움말 · 업데이트 · 제거`, `종료`
  메뉴가 열립니다.
...
Meong Space와 메뉴 막대는 같은 상태 팔레트를 사용합니다. 색만으로 상태를 전달하지
않으며, 모든 상태에는 형태·움직임 또는 VoiceOver 설명이 함께 있습니다.

#### 에이전트별 상태

각 오브젝트의 몸 색과 형태는 그 에이전트 자신의 현재 상태입니다. 상태가 바뀌면 해당
오브젝트도 아래 색과 형태로 즉시 바뀝니다. 부모·자식 관계는 색이 아니라 탄생·흡수,
크기와 가까운 움직임으로 구분합니다.

#### 메뉴 막대 아이콘

메뉴 막대의 단일 아이콘은 모든 에이전트를 한 색으로 집계하지 않습니다. 가장 최근에
상태가 바뀐 에이전트의 색과 형태를 보여주고, 빠른 연속 변화는 짧게 순서대로 표시합니다.
아이콘이 완료나 실패 색인 채 통통 움직일 수 있습니다. 몸 색은 최근 변화, 움직임은
다른 에이전트를 포함해 하나 이상 활동 중이라는 서로 독립된 신호이기 때문입니다.
모든 에이전트의 현재 상태는 Meong Space에서 동시에 확인합니다.

| 색 | 상태 | 함께 보이는 형태 |
| --- | --- | --- |
| 하늘색 | 고요함 | 정지한 점 |
| 청록색 | 활동 중 | 움직임. 동작 줄이기에서는 꺾쇠 |
| 주황색 | 확인 필요 | 고리 |
| 회보라색 | 상태 불확실 | 분절 고리 |
| 옅은 파란색 | 종료됨. 결과 미확인 | 열린 호 |
| 보라색 | 완료. 성공이 명시됨 | 이중 후광 |
| 회색 | 취소됨 | 가로 막대 |
| 빨간색 | 실패 확인 필요 | 마름모 |

메뉴 막대의 파란 외곽 파동은 방금 최상위 agent turn 하나가 끝났다는 별도 신호이며,
전체 작업의 성공을 뜻하지 않습니다. 앱에서는 Meong Space 우상단 `?`를 눌러 실제
색 견본과 형태를 함께 볼 수 있습니다. 사용자가 연 안내는 안내 안의 `×`, `Escape`,
`?` 재클릭 또는 Meong Space 바깥 클릭으로 닫을 때까지 유지됩니다.
...
자동 업데이트는 없습니다. `⌘ Space`로 Terminal을 열고, 아래 블록 전체를 복사해
붙여넣으면 처음 받았던 소스 폴더에서 새 버전을 받고 앱을 다시 설치합니다.
...
마지막에 `Installed agent-meong at`으로 시작하는 줄이 보이고 앱이 다시 열리면 업데이트가 끝난
것입니다. 설치된 연결과 로그인 자동 실행 설정은 보존됩니다. 연결 정의가 바뀌어
`복구 필요`가 보이는 경우에만 복구 버튼을 누르고 Codex의 보안 검토를 다시 완료합니다.
...
| Codex에 `/hooks`가 없음 | ChatGPT·Codex 앱을 최신 버전으로 업데이트하거나 [공식 설치 안내](https://developers.openai.com/codex/cli)로 Codex CLI를 설치·업데이트합니다. |
| `승인 필요` | `Codex 검토 열기`를 누릅니다. 자동으로 열린 `Hooks need review`에서 `Review hooks`를 선택하고 agent-meong 7개를 신뢰합니다. 예외적으로 일반 입력 화면이 열리면 이미 복사된 `/hooks`를 `⌘V` → `Return`으로 엽니다. |
...
| `destination path ... already exists` | `$HOME/agent-meong`을 이전에 받았다면 [업데이트](#업데이트) 명령을 실행합니다. 다른 내용의 폴더라면 지우지 말고 Finder에서 이름을 바꾼 뒤 다시 설치합니다. |
...
권장합니다. **연결 해제는 앱을 삭제하지 않으며, 앱과 로그인 자동 실행은 계속
유지됩니다.** agent-meong이 추가한
...
우클릭 메뉴의 `종료`는 앱을 닫기만 하며 삭제하지 않습니다. 앱, Codex 연결, 로그인 자동
실행과 로컬 데이터를 모두 제거하려면 처음에 받은 `$HOME/agent-meong` 소스 폴더가
필요합니다.

별도 `CODEX_HOME`을 연결한 적이 없고 기본 `~/.codex`만 사용했다면 Terminal을 열고 다음
블록 전체를 붙여넣으세요.
...
`Uninstalled agent-meong and its default Codex connection.`이 보이면 앱과 데이터 제거가
끝난 것입니다. 안전을 위해 소스 checkout은 자동으로 지우지 않습니다. 마지막으로
Finder에서 홈 폴더의 `agent-meong` 폴더를 휴지통으로 옮기면 받아 두었던 소스까지
모두 제거됩니다.
...
lifecycle event 이름, 다른 검토 대기 hook의 집계 개수만 남깁니다. 다른 hook의 이름,
command와 경로는 앱으로 전달하지 않습니다. Codex app-server의 state와 log도 사용자
전용 임시 directory에 격리해 조회 후 삭제하며, agent-meong은 사용자의 Codex database를
직접 열거나 내용을 읽지 않습니다.
