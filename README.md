...
- 짧은 앱 재시작 뒤에도 원래 관찰 시각의 TTL이 남은 실행·확인 필요 객체 복원
...
연결 해제가 성공하면 현재 장면과 재시작 체크포인트도 함께 비운다.
...
재시작 체크포인트도 이 경계 안의 파생 metadata 중 active, attention, uncertain 객체만
사용자 전용 파일에 저장한다. 원문 event와 완료·실패·취소 객체는 저장하지 않으며,
복원 시 원래 관찰 시각을 기준으로 만료시켜 오래된 작업을 실행 중으로 되살리지 않는다.
...
사용자 설정을 건드리지 않는 hook 설치·복구·해제 검증과 실제 Codex App/CLI에서
사람이 확인해야 하는 최소 신뢰 흐름은
[Codex hook 인수 검증](docs/codex-hook-acceptance.md)에 정리되어 있다.
