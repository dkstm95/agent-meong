...
3. 앱이 연 Codex CLI에서 `⌘V` → `Return`을 누르고, `User config`에 표시된
   agent-meong command handler 7개만 활성화하고 신뢰합니다.
...
3. 열린 Terminal에서 `⌘V`와 `Return`을 누릅니다. `User config`에서 status가
   `agent-meong activity [dev.ailab.agent-meong/v5]`인 7개 command handler만 검토해
   활성화하고 신뢰합니다. agent-meong이 승인 상태를 자동으로 다시 확인하며, 메뉴 막대
   아이콘을 다시 열어도 즉시 최신 상태를 조회합니다.
...
- 최상위 작업 차례 하나가 끝나면 메뉴 막대에 푸른 종료 신호가 나타납니다. 이는 전체
  ...
  보입니다. 이 수는 전체 종료 횟수가 아니라 아직 확인하지 않은 최근 가족 수입니다.
...
확인 필요, 불확실, 종료, 완료, 취소, 실패의 개수와 고리, 분절 고리, 열린 호, 이중
후광, 가로 막대, 마름모 상태 문법을 텍스트로 제공합니다. Reduce Motion에서는 움직임을
정지시키는 대신 꺾쇠 표식으로 활동 중 상태를 유지합니다.
...
| `승인 필요` | `Codex 검토 열기`를 누르고 열린 Terminal에서 `⌘V` → `Return` 후 표시된 agent-meong command handler를 신뢰합니다. |
...
연결에서 온 오브젝트, 종료 흔적과 확인 기록만 장면과 재시작 checkpoint에서 지웁니다.
...
`연결 기록 지우기`를 한 번 눌러 합쳐진 로컬 장면과 확인 기록을 비웁니다.
