...
현재는 이 Mac에서 실행되는 OpenAI Codex만 지원합니다. Codex App, ChatGPT 데스크톱
앱 안의 Codex, Codex CLI를 관찰합니다. 일반 ChatGPT 대화와 Codex 원격·웹 작업은
표시하지 않습니다.

설치는 GitHub 소스로만 제공합니다. 다음 환경이 필요합니다.
...
각 오브젝트의 몸 색은 에이전트를 구분하며 화면에 있는 동안 유지됩니다. 상태는 움직임과
몸 주위의 형태로 표시합니다. 메뉴 막대 아이콘은 가장 최근에 바뀐 상태를 색과 형태로
보여줍니다. 아이콘의 통통 움직임은 별도 신호이며, 하나 이상의 에이전트가 활동 중이라는
뜻입니다.

| 메뉴 막대 색 | 상태 | Meong Space 형태 |
| --- | --- | --- |
| 하늘색 | 고요함 | 점 |
| 청록색 | 활동 중 | 움직임 |
| 주황색 | 확인 필요 | 고리 |
...
몸 색은 에이전트의 이름이나 고유 식별자가 아닙니다. 완료, 취소, 실패는 Codex가 결과를
명시한 경우에만 표시합니다. 파란 외곽 파동은 한 번의
